...
local BOOK_RECEIPT_SHOW_COVER_SETTING = "book_receipt_show_cover"
...
local function buildReceipt(ui, state)
    ...
    if statistics and content_mode ~= CONTENT_MODE_HIGHLIGHT_PROGRESS then
        book_total_time_text = string.format(_("Total time spent: %s"), secs_to_timestring(statistics.book_read_time))
        local today_duration = getBookTodayDuration(statistics)
        if today_duration then
            local day_label = getLocalizedDayName(os.time())
            book_today_time_text = string.format(_("Time spent today (%s): %s"), day_label, secs_to_timestring(today_duration))
        end
    end
    ...
    local user_show_cover = G_reader_settings:nilOrTrue(BOOK_RECEIPT_SHOW_COVER_SETTING)
    local show_cover = user_show_cover and not (Device.screen_saver_mode and bg_choice == "book_cover")
    local cover_widget
    ...
    if content_mode == CONTENT_MODE_BOOK_RECEIPT then
        show_cover = user_show_cover and not (Device.screen_saver_mode and bg_choice == "book_cover")
    else
            ...
            local content_menu = {
                ...
                sub_item_table = {
                    ...
                    },
                    {
                        text = _("Show cover"),
                        checked_func = function()
                            return G_reader_settings:nilOrTrue(BOOK_RECEIPT_SHOW_COVER_SETTING)
                        end,
                        callback = function()
                            G_reader_settings:flipNilOrTrue(BOOK_RECEIPT_SHOW_COVER_SETTING)
                        end,
                    },
                    {
                        text = _("Cover scale"),
